...
func DeployPR(ctx context.Context, webhook *webhook.GithubPRWebhook) error {
	...
	hostPort := fmt.Sprintf("%d", 8080+webhook.Number)

	log.Printf("Using host port: %s, container port: 8080", hostPort)

	app := &types.App{
		Name:          fmt.Sprintf("pr-%d", webhook.Number),
		SourcePath:    repoPath,
		HostPort:      hostPort, // Dynamic port: 8081, 8082, etc.
		ContainerPort: "8080",   // Container always uses 8080
	}
	...
	log.Printf("Preview available at: http://localhost:%s", hostPort)
